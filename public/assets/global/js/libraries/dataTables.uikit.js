...
$.extend( true, DataTable.defaults, {
	dom:
		"<'flex items-center justify-between p-5'<''l><''f>>" +
		"<''<''tr>>" +
		"<'flex items-center justify-between  mt-2 p-6'<''i><''p>>",
	renderer: 'uikit'
} );
...
DataTable.ext.renderer.pageButton.uikit = function ( settings, host, idx, buttons, page, pages ) {
	...
	attach(
		$(host).empty().html('<ul class="border divide-x flex font-medium items-center rounded-md text-gray-600"/>').children('ul'),
		buttons
	);
