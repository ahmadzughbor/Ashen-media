...
(function ($) {
  ...
  $.fn.aksFileUpload = function (options) {
    const aks = $(this);
    const selectorIdd=$(this).attr('id');
    ...
    var settings = $.extend(
      {
        ...
        fileTypeError: "تنسيق الملف غير مسموح به.",
        label: "اسحب الملفات وأفلتها أو تصفح"
      },
      options
    );
    ...
    return this.each(function (i) {
      ...
      function error(text) {
        ...
        if ($('.aks-file-upload-error').length > 0) {
          $('.aks-file-upload-error').html(text);
        }else{
      ...
      function fileIconSvg(fileType,fileTypeExtension){
      ...
      function file(file) {
        ...
        if(fileTypeisExtension){
          dataAppend(file);
        }
        console.log(fileTypeisExtension);
        var fileTypeAdd = "";
        ...
        function allOkey() {
          ...
          var fileIcon =  fileIconSvg(file.type,fileTypeExtension);

          function deleteButton() {
            var deleteButton =
              '<div class="aks-file-upload-delete aks-file-upload-delete-without-action" data-delete="' +
              fileName +
          ...
          if (!file.type.match("image.*")) {
            var filehtml =
              ...
              niceButtons +
              "</header></div>";
            var fileprevieww = aks.find(".aks-file-upload-content");
            fileprevieww.append(filehtml);
      ...
      function fileDelete() {
        $(".aks-file-upload-delete-without-action").on("click", function () {
      ...
      // show previouse  image with delete action 


      var filesList=settings.files;
      var previewlist = aks.find(".aks-file-upload-content");
      if(filesList.length){
        // var preview = $("#"+selectorIdd+" .aks-file-upload-content");

        var deleteAction=settings.deleteFileAction;
        var JsonFilesList = JSON.parse(decodeURIComponent(filesList.replace(/&quot;/g,'"')));




        $.each(JsonFilesList,function(index,item){

          var fileTypeExtension = item.file_name.split(".").pop().toLowerCase();

          var fileIcon = fileIconSvg(item.mime_type,fileTypeExtension);

          var filehtml="";
          if (item.mime_type.match("image.*")) {
            filehtml = `<div data-file="` + item.file_name + `" data-file-type="png" class="aks-file-upload-preview " style="height: 240px;">
                      <header class="aks-file-upload-p-header">
                          <div class="aks-file-upload-p-header-icon">
                          ` + fileIcon + `
                          </div>
                          <div class="aks-file-upload-p-header-content"><span class="aks-file-upload-title">`+item.file_name+`</span><span class="aks-file-upload-size">436 KB</span></div>
                          <div class="aks-file-upload-delete-new aks-file-upload-delete delete-file" data-delete="` + item.file_name + `" url="` + deleteAction + item.id + `"><svg width="26" height="26" viewBox="0 0 26 26" xmlns="http://www.w3.org/2000/svg"><path d="M11.586 13l-2.293 2.293a1 1 0 0 0 1.414 1.414L13 14.414l2.293 2.293a1 1 0 0 0 1.414-1.414L14.414 13l2.293-2.293a1 1 0 0 0-1.414-1.414L13 11.586l-2.293-2.293a1 1 0 0 0-1.414 1.414L11.586 13z" fill="currentColor" fill-rule="nonzero"></path></svg></div>
                      </header>
                      <main class="aks-file-upload-p-main">
                          <div class="aks-file-upload-image">
                              <img src="` + item.file_full_path + `" alt="` + item.file_name + `" data-file="` + item.file_name + `"
                                   class="png" style="height: 240px;">
                          </div>
                      </main>
                  </div>`;
          }else{
            filehtml = `<div data-file="` + item.file_name + `" data-file-type="png" class="aks-file-upload-preview aks-no-preview" style="height: 55px;">
                      <header class="aks-file-upload-p-header">
                          <div class="aks-file-upload-p-header-icon">
                          ` + fileIcon + `
                          </div>
                          <div class="aks-file-upload-p-header-content"><span class="aks-file-upload-title">`+item.file_name+`</span><span class="aks-file-upload-size">436 KB</span></div>
                          <div class="aks-file-upload-delete-new aks-file-upload-delete delete-file" data-delete="` + item.file_name + `" url="` + deleteAction + item.id + `">
